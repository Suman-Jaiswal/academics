...
export const deleteBranch = async (branchId) => {
    const querySnapshot = await getDocs(
        query(collection(firestore, version + '-courses'), where('branchId', '==', branchId))
    );
    querySnapshot.forEach((obj) => {
        deleteSlotsByCourseId(obj.id)
            .then(() => console.log('deleted slots'))
            .catch((e) => console.log(e));
        deleteLinksByParentId(obj.id)
            .then(() => console.log('deleted links'))
            .catch((e) => console.log(e));
        deleteDoc(doc(firestore, version + '-courses', obj.id));
    });
    //  delete branch by where branchid matches
    const querySnapshot2 = await getDocs(
        query(collection(firestore, version + '-branches'), where('branchId', '==', branchId))
    );
...
export const fetchSlots = async (registeredCourses) => {
    const querySnapshot = await getDocs(
        query(collection(firestore, version + '-slots'), where('courseCode', 'in', registeredCourses))
    );
    let docs = [];
    querySnapshot.forEach((doc) => {
        docs.push({
...
export const deleteSlotsByCourseId = async (courseId) => {
    const querySnapshot = await getDocs(
        query(collection(firestore, version + '-slots'), where('courseId', '==', courseId))
    );
    querySnapshot.forEach((obj) => {
...
export const deleteCourseById = async (id) => {
    await deleteSlotsByCourseId(id)
        .then(() => console.log('deleted slots'))
        .catch((e) => console.log(e));
    await deleteLinksByParentId(id)
        .then(() => console.log('deleted links'))
        .catch((e) => console.log(e));
...
export const fetchLinks = async (branchId) => {
    const querySnapshot = await getDocs(
        query(collection(firestore, version + '-links'), where('branchId', '==', branchId))
    );
    let docs = [];
    querySnapshot.forEach((doc) => {
        docs.push({
...
export const deleteLinksByParentId = async (parentId) => {
    const querySnapshot = await getDocs(
        query(collection(firestore, version + '-links'), where('parentId', '==', parentId))
    );
    querySnapshot.forEach((obj) => {
